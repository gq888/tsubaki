<template>
  ...
  <div class="row">
    <div>
      <ul class="cardsul flex-col" style="padding-left: 0; width: 100%; height: 2100px; max-width: 500px; margin: auto">
        <li v-for="(item, i) in cards1" :key='i' class="card" style="width:25%; height: 150px; margin: 0">
          <img :src="'./static/' + item + '.jpg'" v-if="item >= 0"
            @click="hitflag && lockflag && clickCard(item, i)" :class="{shanshuo: sign == item}">
          <span v-else-if="cards1[i - 1] >= 4">{{types[cards1[i - 1] % 4] + point[cards1[i - 1] >> 2 - 1]}}</span>
        </li>
      </ul>
    </div>
  </div>
